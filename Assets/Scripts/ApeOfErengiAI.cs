...
public class ApeOfErengiAI : MonoBehaviour
{
    [SerializeField]
    Transform Parent;
    ...
    void PlayMonkeySound()
    {
        ...
        int randy = Random.Range(0,monkeyNoise.Length);


        if (!aud.isPlaying)
        {
            if(aud.clip == monkeyNoise[randy])
            
                randy++;
                if (randy >= monkeyNoise.Length)
                    randy = 0;
            


            aud.clip = monkeyNoise[randy];
            aud.Play();
        }
