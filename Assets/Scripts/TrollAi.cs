...
public class TrollAi : MonoBehaviour
{
    ...
    //You might notice this script is the same exact thing as the goblin script
    //Thats because it is
    ...
    //This will not be implemented yet (ever now)
    //public float fuckYou; //This is the distance where the character aggros if it gets too close and the current target is too far. 
    ...
    float moveDi; //The value that determines the direction of the character(Should never go above 1)
    ...
    void Checkers()
    {
        isWalled = Physics2D.OverlapBox(wallCheckPos.position, WalledHeight, 0, whatIsGround);//This checks if theres a wall in front of it
       
        if (killThisOne == null)
            theChasm = Physics2D.OverlapCapsule(chasmChecker.transform.position, chasmHeight, 0, whatIsGround);//This checks if theres a huge gap beneath it
                        ...
                        //(Try to stay away from any value below 10 for jump height)
    {
        ...
        int hereYouAre = (int)((jumpForce - 5) / 2.5);



        float xPos = Math.Abs(transform.position.x - killThisOne.transform.position.x);

        if (killThisOne.transform.position.y <= transform.position.y + hereYouAre &&
            killThisOne.transform.position.y > transform.position.y
            && xPos < 1)
            return true;
    ...
    void getThatMf()//Moves towards the enemy unless enemy is null
    {
        flipper();
        if (bored < 0)
            manGoTheOthaWay = true;

        //All of these check if the character is moving towards the object

        if (hitColliders.Length == 0 || !killThisOne)//This one just moves in an aimless direction 
        {
            if (moveDi >= 0)
            {
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireCube(wallCheckPos.position, WalledHeight);
